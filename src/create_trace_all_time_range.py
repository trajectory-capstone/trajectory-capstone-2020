...
cab_dict,cab_set = utils.get_cab_details()
print("cab details fetched")

tot_timestamp_cnt,timestamp_dict = utils.get_timestamp_dict(constants.start_date,constants.end_date,
                                                            constants.hours_range,constants.mins_range)
print("timestamp details fetched")
cab_data = pd.concat([utils.read_trace_df(cab_trace,cab_dict) for cab_trace in cab_set])
print("cabs data processed into dataframe")

print("cabs data processing started")
cab_data['datetime'] = cab_data.apply(lambda row: utils.round_off_to_nearest(int(row[3]),constants.time_gap),axis=1)
...
sf_outline = gpd.read_file(constants.sf_shapefile)
tot_loc_stamp_cnt = len(sf_outline.index.to_list()) + 1
sf_crs = utils.get_crs()
print("san francisco shape files read and details fetched")
...
geo_cabs = geo_cabs.to_crs(crs=sf_crs)
sf_outline = sf_outline.to_crs(crs=sf_crs)
print("cabs data processed into geo dataframe")

geo_cabs = gpd.tools.sjoin(geo_cabs,sf_outline,how='left')
